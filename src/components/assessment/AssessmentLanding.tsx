...
interface AssessmentLandingProps {
  ...
  onRequireLogin: (actionAttempted?: string) => void
}
...
export const AssessmentLanding: React.FC<AssessmentLandingProps> = ({
  ...
  const handleStartNewAttempt = () => {
    if (!currentUser) {
      onRequireLogin('start a new assessment')
      return
    }
    onStartNew()
  }

  const handleStartTextAttempt = () => {
    if (!currentUser) {
      onRequireLogin('start the text questionnaire')
      return
    }
    if (onStartText) {
      onStartText()
  ...
  const handleContinuePrevious = async (e: React.FormEvent) => {
    ...
    if (!currentUser) {
      onRequireLogin('resume your previous counseling session')
      return
    }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-5 py-1">
      ...
      {/* Citizen Authentication Status Banner */}
      {isAuthLoading ? (
        <div className="flex items-center justify-center gap-2 py-3 px-4 rounded-2xl bg-slate-50 border border-slate-200 text-xs text-slate-500">
          <Loader2 className="w-4 h-4 animate-spin text-[#003366]" />
          <span>Checking citizen authentication status...</span>
        </div>
      ) : !currentUser ? (
        <div className="bg-gradient-to-r from-amber-50 to-orange-50 border-2 border-amber-300 rounded-2xl p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm animate-in fade-in">
          <div className="flex items-start sm:items-center gap-3.5">
            <div className="w-11 h-11 rounded-2xl bg-amber-100 border border-amber-300 flex items-center justify-center shrink-0 text-amber-800 shadow-2xs">
              <Lock className="w-5 h-5 text-amber-800" />
            </div>
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h3 className="text-sm font-extrabold text-amber-950">
                  Citizen Login Required to Give Test
                </h3>
              ...
              <p className="text-xs text-amber-900/90 mt-1 leading-relaxed">
                You must login with your Citizen account first before taking the assessment. This protects your confidentiality and links your clinical report to your profile.
              </p>
            </div>
          </div>
          <button
            type="button"
            onClick={() => onRequireLogin('give the assessment')}
            className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-[#003366] hover:bg-[#002244] text-white text-xs sm:text-sm font-bold shrink-0 transition-all shadow-sm cursor-pointer active:scale-95"
          >
            <LogIn className="w-4 h-4" />
            <span>Login to Give Test</span>
            <ArrowRight className="w-4 h-4" />
          </button>
        </div>
      ) : (
        <div className="bg-emerald-50/90 border border-emerald-300 rounded-2xl p-3.5 sm:p-4 flex items-center justify-between gap-3 shadow-xs">
          <div className="flex items-center gap-3">
            ...
            <div className="text-left leading-tight">
              ...
              <p className="text-[11px] text-emerald-800/90 mt-1">
                Your test answers and counsellor support will be safely linked to your Citizen profile.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 items-stretch">
        ...
        <div
          onClick={(e) => { e.preventDefault(); handleStartNewAttempt(); }}
          ...
          role="button"
          tabIndex={0}
          onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); handleStartNewAttempt(); } }}
        >
          ...
          <div className="pt-4 mt-4 border-t border-slate-100 flex flex-col sm:flex-row gap-2">
            <button
              ...
              type="button"
              onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleStartNewAttempt(); }}
            ...
            {onStartText && (
              <button
                type="button"
                onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleStartTextAttempt(); }}
            ...
            <form onSubmit={handleContinuePrevious} className="space-y-3 pt-2">
              ...
              <div className="flex items-center justify-between text-[11px] text-slate-500 pt-1">
                ...
                <button
                  ...
                  className="font-mono font-bold text-blue-700 hover:underline bg-blue-50 px-2 py-0.5 rounded cursor-pointer"
                >
                  ST-82K7P4
                </button>
              </div>
              ...
              <div className="pt-3">
                ...
                >
                  ...
                  {!currentUser && <Lock className="w-3.5 h-3.5 text-slate-300" />}
